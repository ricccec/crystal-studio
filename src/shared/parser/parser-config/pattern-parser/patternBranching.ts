...
import { parseAsmPattern, AsmPattern, checkPatternOverlap } from "../../asm-pattern/asmPattern";
...
export function parseBranches(pattern: string): 
    ...
    | { ok: true, branches: AsmPattern[], placeholders: Set<string> }
{
    ...
    const branchSimplePatterns: AsmPattern[] = [];
    for (const branch of branches) {
        ...
        const r = parseAsmPattern(branch);
 ...
 * @returns ParsePatternResult with success status, error message, branches, and placeholders
 */
export function validateBranches(branches: AsmPattern[]): string | undefined {
    ...
    for (let i = 0; i < branches.length; i++) {
        for (let j = (i + 1); j < branches.length; j++) {
            ...
            if (checkPatternOverlap(br1, br2)) {
